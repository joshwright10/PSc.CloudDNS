function ConvertFrom-PScDNSCSCNSRecord {
    <#
    .SYNOPSIS
        Converts a CSC NS Record into one or more PScDNSRecordSet objects.

    .DESCRIPTION
        The ConvertFrom-PScDNSCSCNSRecord function parses a line from a CSC zone export and converts it into a PScDNSRecordSet object.
    ...
    .EXAMPLE
        PS C:\> ConvertFrom-PScDNSCSCNSRecord -InputObject $Records -Domain "example.com"
    ...
    process {
        foreach ($Entry in $InputObject) {
            ...
            if (-not ($RegexResult)) {
                throw  "Entry '$Entry' is not of type 'NS'"
            }
            ...
            elseif ($Entry -match "^@") {
                Write-Verbose -Message "Ignoring root NS records"
            }
            else {
                ...
                $RecordType = [PScDNSRecordType]::NS
